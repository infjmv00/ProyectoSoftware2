...
IdTrabajador char(15) not null unique,
fecha_fin timestamp(6),
constraint FK_TTR_factura foreign key (n_factura) references tblFacturas(n_factura) ON UPDATE CASCADE,
constraint FK_TTR_NIFtrabajador foreign key (cifnif_trabajador) references tblTrabajadores(IdTrabajador) ON UPDATE CASCADE);
...
create table tblTrabajosEjecutados(
